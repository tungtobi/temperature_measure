...
void setup() {
  ...
  client.publish("test/post", "Hello from ESP8266"); //Topic name
  client.subscribe("test/post");
...
//  client.publish("testpost", "Hello from ESP8266 in loop"); //Topic name
  char* postData_char;
  postData_char = toJson(mlx.readAmbientTempC(), mlx.readObjectTempC());
//  postData.toCharArray(postData_char, sizeof(postData_char));
  client.publish("test/post", postData_char);
  
  delay(2000);
}

char* toJson(float envroment, float object) {
  char* s = (char*) malloc(27);
  snprintf(s, 27, "{\"env\":%.2f, \"obj\":%.2f}", envroment, object);
  return s;
}
